...
describe("alchemical wizardly game logic testing", function (){
    ...
    describe("making a new board", function (){
        it("makes a 7x7 board", function (){
            //checking to make sure the board is 7x7
            const board = createBoard();
            //create board w function

            assert.strictEqual(board.length, BOARD_SIZE);
            //check rows to make sure there are 6 of them (BOARD_SIZE = 6)
            assert.strictEqual(board[0].length, BOARD_SIZE);
            //check columns to make sure there are 6 of them
        });
            ...
            for (let y = 0; y < board.length; y++){
                for (let x = 0; x < board[y].length; x++){
                    if (board[y][x] !== null) {
                        empty = false;
                        break;
                        //checking every cell, if at
                        // least 1 is not null the board is not empty
                    }
                }
            }
    ...
    describe("putting runes and checking reactions", function (){
        ...
        it("lets you place a rune", function (){
            const board = createBoard();
            const pos = { x: 2, y: 3 };
            const rune = RUNE_TIERS.FIRE;//want to put a fire
            // rune in specified pos for the test

            const newBoard = placeRune(board, pos, rune);//placing the rune

            assert.strictEqual(newBoard[3][2], rune);
            assert.strictEqual(board[3][2], null); // original board shouldn't
            // change and new board should have the fire rune in the right spot
        });
        //checking the rune matching logic to make sure it is correct
        it("makes a new rune when 3 fire runes line up", function (){
            let board = createBoard();//create board and put two
            // fire runes in them
            board[2][1] = RUNE_TIERS.FIRE;
            board[2][3] = RUNE_TIERS.FIRE;

            const placed = placeRune(board, { x: 2, y: 2 }, RUNE_TIERS.FIRE);
            //place third fire rune in between the other two

            const result = processTurn(placed, { x: 2, y: 2 });
            //process the turn to let the runes match

            assert.deepStrictEqual(result[2][2], RUNE_TIERS.MAGMA);
            //check that the last placed rune (middle) upgraded to magma
            assert.strictEqual(result[2][1], null);
            assert.strictEqual(result[2][3], null);
            //check that the other two runes were removed from the board
        });
        //testing to make sure that chain reaction works
        // when multiple runes upgrade successively in one move in one turn
        it("should chain react when a magma rune is created and causes a volcano rune to be made",
            function (){
            ...
            const placed = placeRune(board, { x: 2, y: 2}, RUNE_TIERS.FIRE);
            //place fire rune in between the other two fire runes
            const result = processTurn(placed, { x: 2, y: 2});
            //process the turn to let the rune chain reaction happen

            assert.deepStrictEqual(result[2][2], RUNE_TIERS.VOLCANO);
            //check that a volcano rune appears in
            // the place where the rune was last placed
            assert.strictEqual(result[2][1], null);
            //check that all the other runes were removed from the board
            assert.strictEqual(result[2][3], null);
        ...
        it("should not place a rune if the cell is already occupied", function()
        {
            ...
            board[pos.y][pos.x] = RUNE_TIERS.FIRE;
            //putting a fire rune down first
            const newBoard = placeRune(board, pos, RUNE_TIERS.AIR);
        ...
        // change the board, apart from being placed
        it("should not change anything else if no match occurs", function(){
            ...
            //processing the turn
            assert.strictEqual(resultBoard[pos.y][pos.x], RUNE_TIERS.FIRE);
            //fire rune should be in the same pos

            let changed = false; //nothing else on the board should have
            //changed and all the other cells should still have value null
    ...
    describe("winning and losing checks", function (){
        it("wins if there is a primordial rune", function (){
            ...
            board[4][4] = RUNE_TIERS.PRIMORDIAL;
            //placing primordial rune in row 4 column 4
            assert.strictEqual(hasWon(board), true);
            //player wins if they make a primordial rune
        });
        //losing test
        it("loses if board is full", function (){
            ...
            for (let y = 0; y < BOARD_SIZE; y++){
                let row = [];
                for (let x = 0; x < BOARD_SIZE; x++){
                    row.push(RUNE_TIERS.FIRE);
                    //making a board full of fire runes
                }
                board.push(row);
            }

            assert.strictEqual(hasLost(board), true);
            //player loses if board is full
        });
        //making sure that the game doesnt lose
        // accidentally even if theres 1 space left
        it("doesnt lose if even one space is still empty", function (){
            ...
            assert.strictEqual(hasLost(board), false);}
            //player shouldnt lose yet because theres still an empty spot on
            // the board
            );
